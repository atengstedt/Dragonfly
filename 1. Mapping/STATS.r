...
setwd("/faststorage/project/Coregonus/Aja/Dragonfly/STATS_mem/")
...
for (i in 1:n) {
    ...
    #extract the mapped reads from the 7th line
    flagstat[i, "Mapped"] <- as.numeric(sub(".*mapped \\(([0-9.]+)%.*", "\\1", a[7])) / 100
    
    #extract the percentage of properly paired reads from the 12th line
    flagstat[i, "Paired"] <- as.numeric(sub(".* properly paired \\(([0-9.]+)%.*", "\\1", a[12])) / 100
}
